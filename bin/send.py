...
parser.add_argument('-D','--dryrunoff', help='Deactivate positioning help',action='store_true')
parser.add_argument('-L','--laserable', help='Set the layer laserable parameter', nargs=1 )
parser.add_argument('-M','--dimension', help='Set an object dimensions', nargs=2 )
parser.add_argument('-S','--save', help='Save VLM', nargs=1 )
parser.add_argument('-c','--cancel', help='Cancel marking',action='store_true')
parser.add_argument('-d','--dryrun', help='Activate positioning help',action='store_true')
parser.add_argument('-H','--height', help='Set the layer height', nargs=1 )
parser.add_argument('-i','--init', help='Init Machine',required=False, action='store_true')
parser.add_argument('-l','--layer', nargs=1, help='Set the layer of a marking object')
...
parser.add_argument('-p','--position', help='Set an object position and rotation', nargs=3 )
parser.add_argument('-q','--qpset', help='Add a local QP set', nargs=4 )
parser.add_argument('-r','--reference', help='Reference Axes',action='store_true')
parser.add_argument('-s','--server', help='Server IP address',required=False)
parser.add_argument('-u','--upload', help='Upload Geometries',action='store_true')
parser.add_argument('-w','--wmove', help='Move the w axis', nargs=1 )
parser.add_argument('-x','--exit', help='Shutdown the blastpit server',action='store_true')
...
if args.object and args.layer:
    ...
    setmolayer.set("layer", args.layer[0] ) # RofinStandard RofinBackground Background Alternate
    tree = ET.ElementTree(setmolayer)
    root = tree.getroot()
    XML = ET.tostring(root, encoding='utf8', method='xml')

    print(XML)
    s.send( struct.pack( 'iii' + str(len(XML)) + 's', 1, BpCommand.kSetMOLayer, len(XML), XML ))
...
if args.object and args.dimension:
    ...
    print(XML)
    s.send( struct.pack( 'iii' + str(len(XML)) + 's', 1, BpCommand.kSetDimension, len(XML), XML ))
...
if args.object and args.position:
    ...
    print(XML)
    s.send( struct.pack( 'iii' + str(len(XML)) + 's', 1, BpCommand.kSetPosValues, len(XML), XML ))
...
if args.bitmap:
        ...
        for y in range( im.size[1] ):
            ...
            if pixelMap[x,y] == 1:
                ...
                if y == im.size[1]-1:
                    ...
                    y2 = y-1
                    if y1 == y2:
                        # For single pixels, create a small line instead of a point
                        y1 = y1 - 0.25
                        y2 = y2 + 0.25
                    polyline = ET.SubElement(root, "POLYLINE")
                    #polyline.set("ID", "POLYLINE0001")
                    polypoint = ET.SubElement(polyline, "POLYPOINT")
                    polypoint.set("TYPE", "LINE")
                    polypoint.text = str(round(x*xfactor,4)) + " " + str(round(y1*yfactor,4))
                    polypoint = ET.SubElement(polyline, "POLYPOINT")
                    polypoint.set("TYPE", "LINE")
                    polypoint.text = str(round(x*xfactor,4)) + " " + str(round((y2+1)*yfactor,4))
            else:
                if lastYPixel != -1:
                    y1 = lastYPixel
                    y2 = y-1
    ...
    tree = ET.ElementTree(drawing)
    print("Uploading XML...")
    s.send( struct.pack( 'iii' + str(len(XML)) + 's', 1, BpCommand.kImportXML, len(XML), XML ))
...
if args.save:
    ...
    s.send( struct.pack( 'iii' + str(len(filename)) + 's', 1, BpCommand.kSaveVLM, len(filename), filename ))
...
if args.zmove:
    ...
    print(XML)
    s.send( struct.pack( 'iii' + str(len(XML)) + 's', 1, BpCommand.kMoveZ, len(XML), XML ))
...
if args.wmove:
    ...
    print(XML)
    s.send( struct.pack( 'iii' + str(len(XML)) + 's', 1, BpCommand.kMoveW, len(XML), XML ))
