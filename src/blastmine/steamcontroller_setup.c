...
#include "steamcontroller_common.h"

void
Debug_DumpHex(const void *pData, size_t count)
{
...
bool
SteamController_Initialize(const SteamControllerDevice *pDevice)
{
	assert(pDevice);
...
static inline void
SteamController_FeatureReportAddSetting(SteamController_HIDFeatureReport *featureReport, uint8_t setting,
					uint16_t value)
{
...
bool SCAPI
SteamController_Configure(const SteamControllerDevice *pDevice, unsigned configFlags)
{
	SteamController_HIDFeatureReport featureReport;
	...
	SteamController_FeatureReportAddSetting(&featureReport, 0x05,
						(configFlags & STEAMCONTROLLER_CONFIG_RIGHT_PAD_HAPTIC_TOUCH) ? 1 : 0);
	SteamController_FeatureReportAddSetting(&featureReport, 0x07,
						(configFlags & STEAMCONTROLLER_CONFIG_STICK_HAPTIC) ? 0 : 7);
	SteamController_FeatureReportAddSetting(
		&featureReport, 0x08, (configFlags & STEAMCONTROLLER_CONFIG_RIGHT_PAD_HAPTIC_TRACKBALL) ? 0 : 7);
	...
	SteamController_FeatureReportAddSetting(&featureReport, 0x31,
						(configFlags & STEAMCONTROLLER_CONFIG_SEND_BATTERY_STATUS) ? 2 : 0);
...
bool SCAPI
SteamController_SetHomeButtonBrightness(const SteamControllerDevice *pDevice, uint8_t brightness)
{
	SteamController_HIDFeatureReport featureReport;
...
bool SCAPI
SteamController_SetTimeOut(const SteamControllerDevice *pDevice, uint16_t timeout)
{
	SteamController_HIDFeatureReport featureReport;
...
bool SCAPI
SteamController_TurnOff(const SteamControllerDevice *pDevice)
{
	if (!pDevice)
		return false;
...
void SCAPI
SteamController_SaveMelodies(const SteamControllerDevice *pDevice, uint8_t startupMelody, uint8_t shutdownMelody)
{
	SteamController_HIDFeatureReport featureReport;
	...
	featureReport.data[4] =
		0x03;  // 0x03 TODO: Find out what these mean. There should be a setting in Steam that changes these.
...
void SCAPI
SteamController_EnableGyro(const SteamControllerDevice *pDevice)
{
	SteamController_HIDFeatureReport featureReport;

	memset(&featureReport, 0, sizeof(featureReport));
	featureReport.featureId = STEAMCONTROLLER_SET_SETTINGS;
	featureReport.dataLen	= 0x15;
	featureReport.data[0]	= 0x32;
	featureReport.data[1]	= 0x84;
	featureReport.data[2]	= 0x03;
	featureReport.data[3]	= 0x18;
	featureReport.data[4]	= 0x00;
	featureReport.data[5]	= 0x00;
	featureReport.data[6]	= 0x31;
	featureReport.data[7]	= 0x02;
	featureReport.data[8]	= 0x00;
	featureReport.data[9]	= 0x08;
	featureReport.data[10]	= 0x07;
	featureReport.data[11]	= 0x00;
	featureReport.data[12]	= 0x07;
	featureReport.data[13]	= 0x07;
	featureReport.data[14]	= 0x00;
	featureReport.data[15]	= 0x30;
	featureReport.data[16]	= 0x14;
	featureReport.data[17]	= 0x00;
	featureReport.data[18]	= 0x2f;
	featureReport.data[19]	= 0x01;
	SteamController_HIDSetFeatureReport(pDevice, &featureReport);
}
