...
BPGui::BPGui(QWidget *parent) : QMainWindow(parent), ui(new Ui::MainWindow)
{
	ui->setupUi(this);
...
void
BPGui::closeEvent(QCloseEvent *event)
{
...
void
BPGui::SetTrayBalloon(QString title, QString text)
{
	trayIcon->showMessage(title, text);
}
void
BPGui::IconActivated(QSystemTrayIcon::ActivationReason reason)
{
	switch (reason) {
...
void
BPGui::UpdateButton(int button, bool state)
{
...
void
BPGui::UpdateDial(int x, int y, int z, int rx, int ry, int rz)
{
...
void
BPGui::Parse(int command, struct BpPacket *netbuf)
{
	typedef struct {
		int command;
		int (BPGui::*function)(struct BpPacket *);
	} netfunc;
	...
	const netfunc functions[] = {{999, &BPGui::LoadSVG}};
...
int
BPGui::LoadSVG(struct BpPacket *netbuf)
{
	(void)netbuf;
...
void
BPGui::GeneratePosHelp()
{
}
int
BPGui::SendXML(QHostAddress dest, pugi::xml_document *svg)
{
