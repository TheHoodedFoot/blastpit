...
def add_container_group(element, element_bbox, lmosxmlgroup, parent_xml, lmosxml):
...
class SvgToVlm(inkex.EffectExtension):
    ...
    def get_bbox(self, element, parent_transform=None):
        ...
        return result(
            x1, y1, x2, y2, width, height, cenx, ceny, tr_centozero, tr_origintozero
        )
    ...
    def process_shape(self, element_array):
        ...
        if is_compound_path(element.path) is True:
            ...
            logging.debug(
                "Transforming compound path " + element.get_id() + " to zero zero"
            )
            ...
            logging.debug(
                "Transforming compound path " + element.get_id() + " with no parent xml"
            )
            ...
            logging.debug(
                "Transforming compound path " + element.get_id() + " with parent xml"
            )
            ...
            path = element.path.transform(
                inkex.Transform(matrix=((1, 0, 0), (0, -1, element_bbox.height)))
                * element_bbox.origintozerozero,
                inplace=False,
            )
        ...
        for segment in path.to_superpath().to_path(curves_only=True):
            ...
            matches = re.match(".*C (.*) (.*) (.*) (.*) (.*) (.*)$", str(segment))
            if matches is not None:
                polyline.bezier(
        ...
        if lmosxmlgroup is None:
            ...
            if parent_xml is None:
                logging.debug("Adding polyline with no lmosxmlgroup or parent xml")
                self.lmosxml.addPolyline(polyline, None, True)
            else:
                logging.debug("Adding polyline with parent xml but no lmosxmlgroup")
                self.lmosxml.addPolyline(polyline, parent_xml)  # , lmosxmlgroup)
        else:
