...
RunTests()
{
	...
	suite->addTest(new CppUnit::TestCaller<SvgTest>(
		"SVG to lines and arcs", &SvgTest::DecomposeTest));
	/* suite->addTest(new CppUnit::TestCaller<SvgTest>( */
	/* 	"Bezier to arcs", &SvgTest::BezierToArcsTest)); */
	suite->addTest(new CppUnit::TestCaller<SvgTest>(
		"SVG to Rofin", &SvgTest::SvgToRofinTest));
	/* suite->addTest(new CppUnit::TestCaller<SvgTest>( */
...
main(int argc, char **argv)
{
	...
	while (1) {
		...
		c = getopt_long(argc, argv, "abc:d:f:os:tn:", long_options,
				&option_index);
	...
	if (verbose_flag) std::cout << "verbose flag is set";
