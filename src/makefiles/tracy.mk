...
$(TRACY_PROFILER_DIR)/Makefile:
	cmake -S $(TRACY_PROFILER_DIR) -B $(TRACY_PROFILER_DIR)/build -DCMAKE_BUILD_TYPE=Release -DLEGACY=$(TRACY_USE_X) -DDOWNLOAD_CAPSTONE=OFF -DFETCHCONTENT_QUIET=OFF

$(TRACY_CAPTURE_DIR)/Makefile:
	cmake -S $(TRACY_CAPTURE_DIR) -B $(TRACY_CAPTURE_DIR)/build -DCMAKE_BUILD_TYPE=Release -DLEGACY=$(TRACY_USE_X) -DDOWNLOAD_CAPSTONE=OFF -DFETCHCONTENT_QUIET=OFF

$(TRACY_CSVEXPORT_DIR)/Makefile:
	cmake -S $(TRACY_CSVEXPORT_DIR) -B $(TRACY_CSVEXPORT_DIR)/build -DCMAKE_BUILD_TYPE=Release -DLEGACY=$(TRACY_USE_X) -DDOWNLOAD_CAPSTONE=OFF -DFETCHCONTENT_QUIET=OFF
