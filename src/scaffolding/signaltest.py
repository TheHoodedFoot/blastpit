...
class Multipass:
    ...
    def run(self):
        ...
        for i in range(1):
            id = blastpy.BpWaitForSignalOrTimeout(self.blast, -20, 2000)
            print("BpWaitForSignalOrTimeout result:", id.id, id.string)

        # for i in range(10):
        #     id = blastpy.BpQueueCommand(self.blast, blastpy.kSelfTest)
        #     blastpy.BpUploadQueuedMessages(self.blast)
        #     result = blastpy.BpWaitForReplyOrTimeout(
        #         self.blast, id.id, myconfig.WS_TIMEOUT_SHORT
        #     )
        #     print("Result of kSelfTest:", result.retval)
        #     time.sleep(2)

        #     id = blastpy.BpWaitForSignalOrTimeout(self.blast, -1, 2000)
        #     print("BpWaitForSignalOrTimeout result:", id.id, id.string)
