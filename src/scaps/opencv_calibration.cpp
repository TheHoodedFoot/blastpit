...
const char *liveCaptureHelp = "When the live video from camera is used as "
...
static void help(char **argv) {
  printf(
      ...
      "(11 by default)\n"
      "     [-fx=<X focal length>]   # focal length in X-dir as an initial "
      "intrinsic guess (if this flag is used, fx, fy, cx, cy must be set)\n"
      "     [-fy=<Y focal length>]   # focal length in Y-dir as an initial "
      "intrinsic guess (if this flag is used, fx, fy, cx, cy must be set)\n"
      "     [-cx=<X center point>]   # camera center point in X-dir as an "
      "initial intrinsic guess (if this flag is used, fx, fy, cx, cy must be "
      "set)\n"
      "     [-cy=<Y center point>]   # camera center point in Y-dir as an "
      "initial intrinsic guess (if this flag is used, fx, fy, cx, cy must be "
      "set)\n"
      "     [-imshow-scale           # image resize scaling factor when "
      "displaying the results (must be >= 1)\n"
      "     [-enable-k3=<0/1>        # to enable (1) or disable (0) K3 "
      "coefficient for the distortion model\n"
...
static void saveCameraParams(
    const string &filename, Size imageSize, Size boardSize, float squareSize,
    float aspectRatio, int flags, const Mat &cameraMatrix,
...
int main(int argc, char **argv) {
  ...
  if (parser.has("fx") && parser.has("fy") && parser.has("cx") &&
      parser.has("cy")) {
  ...
  for (i = 0;; i++) {
    ...
    if (viewScaleFactor > 1) {
      Mat viewScale;
      resize(view, viewScale, Size(), 1.0 / viewScaleFactor,
             1.0 / viewScaleFactor, INTER_AREA);
      imwrite("opencv_calibrated.png", viewScale);
    } else {
      imwrite("opencv_calibrated.png", view);
    }
  ...
  if (!capture.isOpened() && showUndistorted) {
    ...
    for (i = 0; i < (int)imageList.size(); i++) {
      ...
      if (viewScaleFactor > 1) {
        Mat rviewScale;
        resize(rview, rviewScale, Size(), 1.0 / viewScaleFactor,
               1.0 / viewScaleFactor, INTER_AREA);
        imwrite("opencv_calibrated.png", rviewScale);
      } else {
        imwrite("opencv_calibrated.png", rview);
      }
      char c = (char)waitKey();
      if (c == 27 || c == 'q' || c == 'Q')
        break;
